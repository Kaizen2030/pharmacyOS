...
export default function Claims() {
  ...
  const [form, setForm] = useState({
    member_no: '', drug_name: '', quantity: '',
    diagnosis_code: '', fund_type: 'SHIF', amount: ''
  })

  useEffect(() => {
    if (pharmacyId) fetchClaims()
  }, [pharmacyId])
  ...
  async function fetchClaims() {
    ...
    const sha = all.filter(t =>
      t.payment_method?.toLowerCase().includes('sha') ||
      t.payment_method?.toLowerCase().includes('shif') ||
      t.payment_method?.toLowerCase().includes('nhif') ||
      t.payment_method?.toLowerCase().includes('phc') ||
      t.payment_method?.toLowerCase().includes('eccif') ||
      t.payment_method?.toLowerCase().includes('government')
    )
  ...
  const fundTypes = ['ALL', 'SHA', 'SHIF', 'NHIF', 'PHC', 'ECCIF']

  const filtered = claims.filter(c => {
    const matchFund = filterFund === 'ALL' || 
      c.payment_method?.toUpperCase().includes(filterFund)
    ...
    return matchFund && matchFrom && matchTo
  })
  ...
  function buildCSV(rows) {
    const headers = ['Claim ID', 'Date', 'Drug', 'Qty', 'Amount (KES)', 'Fund Type', 'Cashier', 'Status']
    const data = rows.map(c => [
      `SHA-${String(c.id).padStart(4,'0')}`,
  ...
  function downloadAll() {
    downloadCSV(filtered, `sha_claims_${filterFund}_${Date.now()}.csv`)
  }

  function downloadSelected() {
    const rows = filtered.filter(c => selected.includes(c.id))
    if (!rows.length) return alert('Select at least one claim first')
    downloadCSV(rows, `sha_claims_selected_${Date.now()}.csv`)
  }

  function downloadSingle(c) {
    downloadCSV([c], `sha_claim_SHA-${String(c.id).padStart(4,'0')}.csv`)
  }

  if (loading) return <div style={styles.loading}>Loading SHA claims...</div>
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topbar}>
        <div>
          <h2 style={styles.title}>SHA Claims</h2>
          <p style={styles.subtitle}>Track and export government fund claims for Social Health Authority and related programs.</p>
        </div>
        <button style={styles.btnPrimary} onClick={() => setShowForm(true)}>+ New SHA Claim</button>
      </div>
      ...
      <div style={styles.filterBox}>
        <div style={styles.filterRow}>
          <span style={styles.filterLabel}>Fund type:</span>
          {fundTypes.map(f => (
            <button 
              key={f} 
      ...
      <div style={styles.card}>
        {filtered.length === 0 ? (
          <div style={styles.empty}>
            ...
            <p>No SHA claims found for this filter.</p>
            <p style={{ fontSize: '11px', color: '#9ca3af', marginTop: '4px' }}>
              Sales with payment "SHA", "SHIF", "NHIF", "PHC" will appear here.
            </p>
          </div>
        ) : (
      ...
      {showForm && (
        <div style={styles.overlay}>
          <div style={styles.modal}>
            <h3 style={styles.modalTitle}>File New SHA Claim</h3>
            <div style={styles.staffNote}>
              Recording under <strong>{operatorName}</strong>{operatorRole ? ` (${operatorRole})` : ''}.
            </div>
            <div style={styles.formGrid}>
              {[
                { label: 'SHA Member No.', key: 'member_no', placeholder: 'Membership number' },
                { label: 'Drug Name', key: 'drug_name', placeholder: 'Drug name as dispensed' },
            ...
            <div style={styles.modalFooter}>
              ...
              <button 
                ...
                onClick={async () => {
                  if (!form.member_no || !form.drug_name || !form.amount) {
                  ...
                  const { error } = await insertRowsWithSchemaFallback('sales_ledger', [{
                    ...
                    payment_method: form.fund_type,
                    customer_name: form.member_no,
                  ...
                  alert('SHA Claim filed!')
                  setShowForm(false)
                  setForm({ member_no: '', drug_name: '', quantity: '', diagnosis_code: '', fund_type: 'SHIF', amount: '' })
                  fetchClaims()
                }}
              >
                File Claim
...
const styles = {
  ...
  filterLabel: { fontSize: '12px', color: '#6b7280', fontWeight: '600', minWidth: '80px' },
